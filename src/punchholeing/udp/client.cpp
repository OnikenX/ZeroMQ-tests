...
int main(int argc, char* argv[])
{
    ...
    while (1)
    {
        ...
        if (recvfrom(s, &buf, sizeof(buf), 0, (struct sockaddr*)(&si_other), reinterpret_cast<socklen_t *>(&slen)) == -1)
            diep("recvfrom");
        ...
        if (server.host == si_other.sin_addr.s_addr && server.port == (short)(si_other.sin_port))
        {
            ...
            printf("Added peer %s:%dn", inet_ntoa(si_other.sin_addr), ntohs(si_other.sin_port));
            printf("Now we have %d peersn", n);
        ...
        else
        {
            ...
            for (i = 0; i < n; i++)
            {
                ...
                if (peers[i].host == buf.host && peers[i].port == (short)(buf.port))
                {
                    ...
                    printf("Received from peer %d!n", i);
                    break;
                }
            }
