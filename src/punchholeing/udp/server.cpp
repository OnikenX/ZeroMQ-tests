...
int main(void)
{
    ...
    // server cannot be behind a NAT.    // si_me stores our local endpoint. Remember that this program
    // has to be run in a network with UDP endpoint previously known
    // and directly accessible by all clients. In simpler terms, the
    ...
    while (true)
    {
        ...
        if (recvfrom(s, buf, BUFLEN, 0, (struct sockaddr*)(&si_other), reinterpret_cast<socklen_t *>(&slen)) == -1)
            diep("recvfrom");
        ...
        for (i = 0; i < n; i++)
        {
            ...
            for (j = 0; j < n; j++)
            {
                ...
                printf("Sending to %s:%dn", inet_ntoa(si_other.sin_addr), ntohs(si_other.sin_port));
        ...
        printf("Now we have %d clients", n);
